...
<center><h1 style="margin-left:25px;color:white">Selected Item</h1></center>
<?php
...
$sql = "SELECT SLNO,DATE_OF_RECEIPT,DESCRIPTION,NO_ITEMS_RECEIVED,RATE,TOTAL_COST,TAX_PERCENTAGE,SUPPLIER_DETAILS,FUNDING_AGENCY,STOCK_NO,STATE,LOCATION FROM equipments WHERE SLNO = '".$q."' ";
$result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) > 0) {
  echo "  <div id='tab' style='margin-left:200px;color:white'><table id='t' class='table  table-bordered' style='max-width:50%;'>
    <thead>
      <tr>
        <th>SLNO</th>
...
";
 $i=1;
    ...
    $row = mysqli_fetch_assoc($result);
        echo "<tr><td>" . $row["SLNO"]. " </td><td>" . $row["DATE_OF_RECEIPT"]. "</td><td> " . $row["DESCRIPTION"]. "</td><td>" . $row["NO_ITEMS_RECEIVED"]. "</td><td>" .$row["RATE"]. "</td><td>" .$row["TOTAL_COST"]. "</td><td>".$row["TAX_PERCENTAGE"]."</td><td>" .$row["SUPPLIER_DETAILS"]. "</td><td>" .$row["FUNDING_AGENCY"]. "</td><td>" .$row["STOCK_NO"]. "</td><td><div id='".$i."'>" .$row["STATE"]. "</div></td><td>" .$row["LOCATION"]. "</td></tr>" ;
        if( $row["STATE"] == "active"){
      ?>
      <script type="text/javascript">
        val = <?php echo $row["NO_ITEMS_RECEIVED"]; ?>;
        console.log("hahah");
        console.log(val);
      </script>
        <script type="text/javascript">
...
?>
  <br>
<div id="noff" style="height: 250px;width:50%;margin-left:400px;">
  <form  class="example" method="post" action="issue_item.php" onsubmit="return rcheck()" style="margin:auto;">
  <h3 style="color:white;">Current Location:</h3><input id="skill_id" type="text" value ='<?php echo $row["LOCATION"] ?>' readonly  name="l" placeholder="ID" required=""><br>
  <br>
  <h3 style="color:white;">Issue to</h3>
  <input type="text" id="newl" name="nl" placeholder="Enter new location">
  <div style="float: right;margin-top:-154px;">
  <h3 style="color:white;">Number of items</h3>
  <input  type="number" id="no" placeholder="number of items" name="noi">
  <input  type="number" id="sl"  value ='<?php echo $row["SLNO"] ?>' name="slno" hidden>
    <input  type="text" id="sn"  value ='<?php echo $row["DESCRIPTION"] ?>' name="sname" hidden>
  ...
  <br>
  <br>
  <button style="width:100px;border-radius:10px;border:none;outline:none;" id='is' type='submit' name='issue' >ISSUE</button>
  <button style="width:100px;border-radius:10px;border:none;outline:none;margin-left:15px;" id='is'  onclick="goBack()" >cancel</button>
  <script>
function goBack() {
  window.history.back();
}
...
function rcheck(){
  if(!(document.getElementById('no').value <= val && document.getElementById('no').value > 0 ))
  {
    console.log("ee");
    console.log(val);
          toastr["error"]("Invaid Number of items ", "Error");
          return false;
  }
else if (document.getElementById('newl').value == 0 || document.getElementById('no').value == 0) {
