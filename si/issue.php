...
<center><h1 style="margin-left:25px;color:white">Stock List</h1></center>
<form  class="example" method="post" action="issue.php" style="margin-left:200px;float:left;margin-top:-50px;max-width:300px;">
...
<a href="issue.php"><button id="iss" style="margin-left:200px;float:left;" value="Refresh Page">View All</button></a><br><br><br>
</div>
<?php
...
if ($result->num_rows > 0) {
  echo " <br> <form name='exampl' action='issueitem.php' method='post'><div id='tab' style='margin-left:200px;color:white'><table id='t' class='table  table-bordered' style='max-width:50%;'>
    <thead>
      <tr>
        <th>Issue</th>
        <th>SLNO</th>
...
<?php
        ...
        echo "<tr><td><center><input name='rad' value = '".$row["SLNO"]."' type='radio'></center></td><td>" . $row["SLNO"]. "</td><td>" . $row["DATE_OF_RECEIPT"]. "</td><td> " . $row["DESCRIPTION"]. "</td><td>" . $row["NO_ITEMS_RECEIVED"]. "</td><td>" .$row["RATE"]. "</td><td>" .$row["TOTAL_COST"]. "</td><td>".$row["TAX_PERCENTAGE"]."</td><td>" .$row["SUPPLIER_DETAILS"]. "</td><td>" .$row["FUNDING_AGENCY"]. "</td><td>" .$row["STOCK_NO"]. "</td><td><div id='".$i."'>" .$row["STATE"]. "</div></td><td>" .$row["LOCATION"]. "</td></tr>" ;
    ...
    echo "</tbody></table></div><br><center><button id='is' type='submit' name='issue' disabled>ISSUE</button><br><br></form></center>";
    ?>
<script type="text/javascript">
  $(function() {
        document.getElementById("all").style = "display:block";
    $('input[type="radio"]').on('click', function() {
            document.getElementById("is").style ="cursor:pointer;opacity:1;";
            document.getElementById("is").removeAttribute("disabled");
    });
 });
</script>
    <?php
...
$sql = "SELECT SLNO,DATE_OF_RECEIPT,DESCRIPTION,NO_ITEMS_RECEIVED,RATE,TOTAL_COST,TAX_PERCENTAGE,SUPPLIER_DETAILS,FUNDING_AGENCY,STOCK_NO,STATE,LOCATION FROM equipments WHERE STATE = 'active' AND NO_ITEMS_RECEIVED > 0 ";
$result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) > 0) {
  echo "  <form name='exampl' action='issueitem.php' method='post'> <div id='tab' style='margin-left:200px;color:white'><table id='t' class='table  table-bordered' style='max-width:50%;'>
    <thead>
      <tr>
        <th>Issue</th>
        <th>SLNO</th>
...
<?php
    ...
    echo "</form></tbody></table></div><center><br><button id='is' type='submit' name='issue' disabled>ISSUE</button></form></center><br>";
    ?>
<script type="text/javascript">
  $(function() {
    $('input[type="radio"]').on('click', function() {
...
function myfs(){
    toastr["success"]("Stock Issued Successfully!");
}
function myfss(){
    toastr["success"]("Stock Issued Successfully!");
}
function myff(){
    toastr["error"]("Error in Issueing stock!, please try again");
}
toastr.options = {
  "closeButton": false,
  "debug": false,
