...
$sql = "SELECT * FROM issued WHERE id = '".$q."' ";
$result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) > 0) {
  echo "<div id='tab' style='margin-left:400px;color:white'><table id='t' class='table  table-bordered' style='max-width:50%;'>
    <thead>
      <tr>
		<th>SLNO</th>
        <th>NAME</th>
...
?>
  <br>
<div id="noff" style="height: 250px;margin-left:500px;">
  <form  class="example" method="post" action="transfer_item.php" onsubmit="return rcheck()" style="margin:auto;">
  <h3 style="color:white;">Current Location:</h3><input id="skill_id" type="text" value ='<?php echo $row["ISSUED_TO"] ?>' readonly  name="loc" placeholder="ID" required=""><br>
  <br>
  <h3 style="color:white;">Issue to</h3>
  <input type="text" id="newl" name="nloc" placeholder="Enter new location">
  <div style="float: right;margin-top:-154px;">
  <input  type="number" id="sl"  value ='<?php echo $row["ID"] ?>' name="idd" hidden>
  <input  type="text" id="nal"  value ='<?php echo $row["NAME"] ?>' name="nam" hidden>
  <input  type="number" id="noil"  value ='<?php echo $row["NO_ITEMS_ISSUED"] ?>' name="noit" hidden>
  <br>
  <br>
  <br>
  <br>
  <br>
  <button style="width:120px;border-radius:10px;border:none;outline:none;" id='is' type='submit' name='transfer' >TRANSFER</button>
  </div>
  </form>
  </div>
...
function rcheck(){
  if (document.getElementById('newl').value == 0 ) {
